"
  analysis_topic_detection.R
   Author:
      Jake J. Dalli (jake.dalli.10@um.edu.mt)
    Description:
      In this script we train an LDA model:
        1. We load the data and clean it.
        2. We perform the k selection process described in the paper.
        3. We train the LDA model and build the covariance matrix.
        4. We output data to be used within the analysis_detected_topics.R
    !! WARNING !! 
      This script takes VERY long to terminate and consumes multiple CPU cores.
      The LDA evaluation process takes approximately 18 hours to complete.
      We are training 100+ models with quite a lot of data (albeit in parallel)
      Your PC may freeze up when running it. You have been warned.
      Instead of running it, we suggest you open this script in R and load the environment from within the ./env/ directory.
    !! WARNING !! 
"
library(topicmodels)
library(tm)
